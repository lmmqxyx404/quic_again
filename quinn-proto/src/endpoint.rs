...
use crate::{
    ...
    token::{ResetToken, RetryToken, TokenDecodeError},
    transport_parameters::{PreferredAddress, TransportParameters},
    ConnectionIdGenerator, Side, Transmit, TransportError, INITIAL_MTU, MAX_CID_SIZE,
    ...
    fn handle_first_packet(
        ...
        let (retry_src_cid, orig_dst_cid): (Option<ConnectionId>, ConnectionId) =
            if header.token.is_empty() {
                ...
                tracing::debug!("handle_first_packet former perhaps is retransmit_all_for_0rtt");
...
pub enum ConnectError {
    ...
    EndpointStopping,
    /// 4. The remote [`SocketAddr`] supplied was malformed
    ///
